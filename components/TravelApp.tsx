...
type PageName = 'today' | 'overview' | 'documents' | 'outfit' | 'packing';
...
function Photo({ src, alt }: { src: string; alt: string }) {
  if (!src) {
    return <div className="placeholder">穿搭示意圖待上傳</div>;
  }
...
export default function TravelApp({ days }: { days: TravelDay[] }) {
  ...
  const [overviewChapter, setOverviewChapter] = useState('全部');
  const [checkedItems, setCheckedItems] = useState<string[]>([]);

  useEffect(() => {
    if (!publishedDays.length || selectedDate) return;

    const canadaToday = getTodayCanada();

    const exact = publishedDays.find(
      (day) => getDateKey(day.date) === getDateKey(canadaToday)
    );

    if (exact) {
      setSelectedDate(exact.date);
      return;
    }

    const tripStart = new Date('2026-08-13T00:00:00-07:00');
    const tripEnd = new Date('2026-08-28T23:59:59-07:00');
    const now = new Date();

    if (now < tripStart) {
      setSelectedDate(publishedDays[0].date);
      return;
    }

    if (now > tripEnd) {
      setSelectedDate(publishedDays[publishedDays.length - 1].date);
      return;
    }

    setSelectedDate(publishedDays[0].date);
  }, [publishedDays, selectedDate]);
  ...
  const overviewDays = useMemo(() => {
    if (overviewChapter === '全部') return publishedDays;
  ...
  const switchPage = (nextPage: PageName) => {
    setPage(nextPage);
    ...
    return (
      <main>
        ...
        <section className="contentPage">
          ...
          <h2>目前沒有可顯示的行程</h2>
          <p>請確認 Google Sheets 中至少有一天的 published / status 設為發布。</p>
        </section>
      </main>
    );
  }
  ...
  return (
    <main>
      ...
      {page === 'today' && (
        <TodayPage
          day={selectedDay}
          onOpenOutfit={() => openOutfit(selectedDay.date)}
        />
      )}
    ...
    <div className="todayPage">
      <section className="todayHero">
        ...
        <div className="levelLine">
          <span>今日穿著</span>
          <strong>LEVEL {day.level}｜{levelName(day.level)}</strong>
        </div>
      ...
      <section className="routeSection">
        ...
        <div className="timeline">
          {day.itinerary.map((item, index) => (
            <div className="stop" key={`${item.time}-${item.place}-${index}`}>
              <time>{item.time}</time>
  ...
  return (
    <section className="contentPage">
      ...
      <div className="emptyCard">
        ...
        <strong>文件區已準備好</strong>

        <p>
          這裡之後會放航班、住宿、租車、景點、保險等重要憑證與下載連結。
        </p>
      </div>
    </section>
  );
...
function OutfitPage({
  ...
  return (
    <section className="contentPage">
      <p className="eyebrow">WARDROBE GUIDE</p>
      <h2>穿搭指南</h2>
      ...
      <div className="chapterTabs">
        {days.map((item) => (
          <button
            key={item.date}
            className={item.date === day.date ? 'active' : ''}
            onClick={() => {
      ...
      <div className="outfitIntro">
        ...
        <p>
          白天 {day.dayTemp} ／ 早晚 {day.nightTemp} ／ LEVEL {day.level}
        </p>
      </div>
      ...
      <div className="outfitRows" style={{ marginTop: 36 }}>
        <div>
          <b>天氣</b>
...
function PackingPage({
  ...
  return (
    <section className="contentPage">
      ...
      <p style={{ marginBottom: 28, color: '#687168' }}>
        已完成 {completed} / {total}
      </p>
